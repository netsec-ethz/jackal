...
func (s *outStream) dial(ctx context.Context) error {
	isSCIONAddress, remote := rainsLookup(s.cfg.remoteDomain)
	if isSCIONAddress {
		sess, err := s.dialer.DialQUIC(s.cfg.scion, remote, s.cfg.localDomain, s.cfg.remoteDomain)
		if err != nil {
			return err
		}
		...
		s.tr = transport.NewQUICSocketTransport(sess, biStream, s.cfg.keepAlive)
	} else {
		conn, err := s.dialer.DialTCP(ctx, s.cfg.remoteDomain)
		if err != nil {
			return err
		}
